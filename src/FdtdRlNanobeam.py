""" this code establishes the real-time port b/t FDTD and Python to enable state and reward passes
    for optimization of PCSEL by RL/BO. NOEL, Renjie Li, Sixuan Mao, June 2025
"""

#from home/ondemand/220019012/.conda/envs/bayesopt/lib/python3.11/site-packages import meep as mp
import meep as mp
...
sys.path.append("/home/software/lumerical/v211/api/python/")   # Default windows lumapi path
#import lumapi as lp
...
class FdtdRlNanobeam():
    ...
    def adjustdesignparams(self, netDt1,netDt2,netDt3,netDt4,netDt5,netDnp_index,netDsub_index,netDact_index,netDlattice_spacing,netDradius):
        ...
        hole_radius = 0.1 +netDradius # radius of the air holes (in microns)
        lattice_spacing = 0.3 +netDlattice_spacing # spacing between lattice points (in microns)
        active_layer_thickness = 0.5 +netDt3 # thickness of the active layer (in microns)
        pc_thickness = 0.5+netDt4
        n_sub_thickness = 0.2 +netDt1 # thickness of n-substrate layer (in microns)
        n_clad_thickness = 0.3 +netDt2 # thickness of n-cladding layer (in microns)
        p_clad_thickness = 0.3 +netDt5 # thickness of p-cladding layer (in microns)

        # Define the materials
        n_inp = 3.4 +netDact_index # refractive index of InP
        n_air = 1.0  # refractive index of air
        n_substrate = 3.0 +netDsub_index # refractive index of substrate
        n_cladding = 3.2 +netDnp_index # refractive index of cladding (n-type)
        p_cladding = 3.2  +netDnp_index # refractive index of cladding (p-type)
        ...
        # Define the source
        source_position = mp.Vector3(0, 0, -n_clad_thickness - active_layer_thickness / 2 - n_sub_thickness / 2)
        source = mp.Source(mp.GaussianSource(fcen, fwidth= df), component=mp.Hz,
        ...
        resonance_top = mp.FluxRegion(
            center=mp.Vector3(0.45 * n_side * lattice_spacing, 0.45 * n_side * lattice_spacing, n_sub_thickness / 2),
        ...
        pt = mp.Vector3(
            z=-n_sub_thickness / 2 - n_clad_thickness - active_layer_thickness - pc_thickness / 2)  # at the PC layer
        ...
        farfield_box = mp.FluxRegion(center=mp.Vector3(0, 0, 0), size=mp.Vector3(500, 500, 0), direction=mp.Z)
        # Use the add_flux function to compute the E field on the large sphere
        E_far = sim.add_flux(fcen, 0, 1, farfield_box)
        ...
        sim.run(mp.in_volume(mp.Volume(center=mp.Vector3(x=pc_x / 8, y=pc_y / 8,
                                                         z=-n_sub_thickness / 2 - n_clad_thickness - active_layer_thickness / 2),
                                       size=mp.Vector3(0.7 * pc_x, 0.7 * pc_y))),
        ...
        ey_data = ey_data[4, :, :].squeeze()
        #print("e field:", ey_data)
        ...
        #plt.imshow(
        #    np.flipud(np.transpose(np.abs(ey_data))),
        #    cmap='jet',
        ...
        far_field_data = sim.get_farfield(E_far, mp.Vector3(550,550,1))
        ...
        div_angle = angles[indices[-1]] - angles[indices[0]]

        # compute the far field
        # half side length of far-field square box
        #r = 1e3  # 1mm
        # resolution of far fields (points/??m)
        ...
        print("Mode volume:", V)
        print("Divergence Angle: ", div_angle)

        return Q, freq, max(Pz), V, div_angle
